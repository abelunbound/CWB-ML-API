...
#     """
#     Prepare SQL queries and values for creating a table and inserting data.
...
# def prepare_sql_queries_and_values(column_definitions, table_name, data):
#     """
#     Prepare SQL queries and values for creating a table and upserting data based on applicant_id.
...
#     # Define the unique column for upsert operations
#     unique_column = "applicant_id"
    
#     # Boolean columns for conversion
#     boolean_columns = [col for col, type_def in column_definitions.items() if type_def == 'BOOLEAN']
    
#     # Create table query with unique constraint on applicant_id
#     table_columns_definition = ",\n            ".join(f"{col} {data_type}" for col, data_type in column_definitions.items())
#     constraints = f",\n            CONSTRAINT {table_name}_pk PRIMARY KEY ({unique_column})"
...
#     # Create the update clause for all columns except the unique column
#     non_key_columns = [col for col in columns if col != unique_column]
